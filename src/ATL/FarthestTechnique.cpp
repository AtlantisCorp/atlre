...
namespace Atl
{
    std::size_t FarthestTechnique::sort(const RenderNode& node, const Camera& camera, const Frustum& frustum, RenderTechnique::NodesMap& nodes) const
    {
        if (!node.isVisible())
            return 0;
